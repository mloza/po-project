...
public class Administrator extends Person implements worker{
	...
	public void run() {
		...
		String[] actionList = { "", "Utwórz nowe konto", "Usuń konto",
				"Wyświetl konto" };
		String[] typeList = { "", "Administrator", "Lekarz", "Pielegniarka",
				"Superciec" };
				...
				try {
					...
					while (search.readRecord()) {
						if (log.equals(search.get("LOGIN"))) {
							if(ans.contains(log))
								nowy = MessageFormat.format("{0} {1} {2} {3} {4}\n",
										search.get("TYPE"), search.get("LOGIN"), search.get("NAME"),
										search.get("SURNAME"), search.get("BIRTHDATE"));
						...
						if (typ.equals(search.get("TYPE"))) {
							nowy = MessageFormat.format("{0} {1} {2} {3} {4}\n",
									search.get("TYPE"), search.get("LOGIN"), search.get("NAME"), search.get("SURNAME"),
									search.get("BIRTHDATE"));
							if(!ans.contains(nowy)){
							ans =  ans + nowy;
							}
						}
				...
				try {
					...
					while (search.readRecord()) {
						if (txt.equals(search.get("LOGIN"))) {
							if (ciph.getCipher(haslo).equals(search.get("PASSWORD"))) {
								File in = new File("workers.csv");
					...
					try {
						...
						if (can) {
							CsvWriter saveNew = new CsvWriter(new FileWriter(
									of, true), ',');
							saveNew.write(login);
							saveNew.write(ciph.getCipher(haslo));
							saveNew.write(typ);
							saveNew.write(imie);
							saveNew.write(nazwisko);
							saveNew.write("01-01-1999");
							saveNew.endRecord();
							saveNew.close();
						}
